component {
	...
	*/
	...
	//This datasource should be different for development and production systems because when in development, the entire database can be recreated and we don't want that to happen in production!
	this.datasource = "auth";
	
	//By default, we will tell the ORM to 'update' the database with any changes it find in mapping. This is the default so that we do not accidentally recreate the whole database and whipe out data 
	this.ormsettings={cfclocation="model/orm",useDBforMapping=false,savemapping=true};
